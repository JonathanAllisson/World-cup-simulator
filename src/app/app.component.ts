...
export class AppComponent implements OnInit {
  constructor(service: TeamsService) {
    service.getGroups().subscribe(groups => {
      this.groups = Object.values(groups)[1];
      console.log(JSON.stringify(this.groups))
      this.populateKnockouts(this.groups)
    })
  }
  ...
  changeQuarter(lastMatch: number, winner: Team) {
    ...
    let isTeamOneSide = this.isTeamOneSideNextMatch(lastMatch);
  ...
  getBackgroundStyle(team: any) {
    return {'background-image':'url(' + '../assets/Teams/' + team?.img +')'}
  }
